DE_preparation <- function(){
  ...
  raw.data.9 = read.csv('historic_data/DE_2008-2009.csv')
  raw.data.10 = read.csv('historic_data/DE_2009-2010.csv')
  raw.data.11 = read.csv('historic_data/DE_2010-2011.csv')
  raw.data.12 = read.csv('historic_data/DE_2011-2012.csv')
  raw.data.13 = read.csv('historic_data/DE_2012-2013.csv')
  raw.data.14 = read.csv('historic_data/DE_2013-2014.csv')
  raw.data.15 = read.csv('historic_data/DE_2014-2015.csv')
  raw.data.16 = read.csv('historic_data/DE_2015-2016.csv')
  raw.data.17 = read.csv('historic_data/DE_2016-2017.csv')
  # get the latest available data 
  raw.data.18 = read.csv("http://www.football-data.co.uk/mmz4281/1718/D1.csv")
  
  n.clubs=length(unique(na.omit(raw.data.17$HomeTeam)))
  ...
  next.matches=read.csv("yearly_updated_data/DE_fixtures_2017-2018.csv", sep = ";")
  next.matches=next.matches[c((n.games+1):(n.games+(n.clubs/2))),]
  
  fixtures=as.data.frame(matrix(rep(0,ncol(raw.data.18)*(n.clubs/2)),nrow=(n.clubs/2)))
  names(fixtures)=names(raw.data.18)
  
  #filling the dataframe with coming weeks matches
  fixtures$Div=rep("D1",nrow(fixtures))
  ...
  # get the club names right
  raw.data.18$HomeTeam=str_replace_all(raw.data.18$HomeTeam, fixed("Hannover96"),"Hannover")
  raw.data.18$AwayTeam=str_replace_all(raw.data.18$AwayTeam, fixed("Hannover96"),"Hannover")
  raw.data.18$HomeTeam=str_replace_all(raw.data.18$HomeTeam, fixed("BayerLeverkusen"),"Leverkusen")
  raw.data.18$AwayTeam=str_replace_all(raw.data.18$AwayTeam, fixed("BayerLeverkusen"),"Leverkusen")
  raw.data.18$HomeTeam=str_replace_all(raw.data.18$HomeTeam, fixed("HerthaBSCBerlin"),"Hertha")
  raw.data.18$AwayTeam=str_replace_all(raw.data.18$AwayTeam, fixed("HerthaBSCBerlin"),"Hertha")
  raw.data.18$HomeTeam=str_replace_all(raw.data.18$HomeTeam, fixed("FCSchalke04"),"Schalke04")
  raw.data.18$AwayTeam=str_replace_all(raw.data.18$AwayTeam, fixed("FCSchalke04"),"Schalke04")
  raw.data.18$HomeTeam=str_replace_all(raw.data.18$HomeTeam, fixed("FSVMainz05"),"Mainz")
  raw.data.18$AwayTeam=str_replace_all(raw.data.18$AwayTeam, fixed("FSVMainz05"),"Mainz")
  raw.data.18$HomeTeam=str_replace_all(raw.data.18$HomeTeam, fixed("Bor.Monchengladbach"),"M'gladbach")
  raw.data.18$AwayTeam=str_replace_all(raw.data.18$AwayTeam, fixed("Bor.Monchengladbach"),"M'gladbach")
  raw.data.18$HomeTeam=str_replace_all(raw.data.18$HomeTeam, fixed("VfBStuttgart"),"Stuttgart")
  raw.data.18$AwayTeam=str_replace_all(raw.data.18$AwayTeam, fixed("VfBStuttgart"),"Stuttgart")
  raw.data.18$HomeTeam=str_replace_all(raw.data.18$HomeTeam, fixed("VfLWolfsburg"),"Wolfsburg")
  raw.data.18$AwayTeam=str_replace_all(raw.data.18$AwayTeam, fixed("VfLWolfsburg"),"Wolfsburg")
  raw.data.18$HomeTeam=str_replace_all(raw.data.18$HomeTeam, fixed("FCAugsburg"),"Augsburg")
  raw.data.18$AwayTeam=str_replace_all(raw.data.18$AwayTeam, fixed("FCAugsburg"),"Augsburg")
  raw.data.18$HomeTeam=str_replace_all(raw.data.18$HomeTeam, fixed("SCFreiburg"),"Freiburg")
  raw.data.18$AwayTeam=str_replace_all(raw.data.18$AwayTeam, fixed("SCFreiburg"),"Freiburg")
  raw.data.18$HomeTeam=str_replace_all(raw.data.18$HomeTeam, fixed("HamburgerSV"),"Hamburg")
  raw.data.18$AwayTeam=str_replace_all(raw.data.18$AwayTeam, fixed("HamburgerSV"),"Hamburg")
  raw.data.18$HomeTeam=str_replace_all(raw.data.18$HomeTeam, fixed("EintrachtFrankfurt"),"EinFrankfurt")
  raw.data.18$AwayTeam=str_replace_all(raw.data.18$AwayTeam, fixed("EintrachtFrankfurt"),"EinFrankfurt")
  raw.data.18$HomeTeam=str_replace_all(raw.data.18$HomeTeam, fixed("BorussiaDortmund"),"Dortmund")
  raw.data.18$AwayTeam=str_replace_all(raw.data.18$AwayTeam, fixed("BorussiaDortmund"),"Dortmund")
  raw.data.18$HomeTeam=str_replace_all(raw.data.18$HomeTeam, fixed("1899Hoffenheim"),"Hoffenheim")
  raw.data.18$AwayTeam=str_replace_all(raw.data.18$AwayTeam, fixed("1899Hoffenheim"),"Hoffenheim")
  ...
  raw.data.3$Date = as.Date(raw.data.3$Date, '%d/%m/%y')         
  ...
  ## Shots on target (HST and AST) not available for seasons 3,4,5 and 6
  ...
  get_goals_scored=function(playing_stat){
    ...
    goalsscored = matrix(rep(0,2*(n.clubs*(n.clubs-1))),ncol=((n.clubs-1)*2))
    teamnames=unique(playing_stat$HomeTeam)
    
    # count goals at Home and Away and create cumulative total per matchweek
    for (t in teamnames){
      HTGS=matrix(rep(0,2*(n.clubs-1)),ncol = 2)
      ATGS=matrix(rep(0,2*(n.clubs-1)),ncol = 2)
  ...
  get_goals_conceded=function(playing_stat){
    # Create a dictionary with team names as keys
    goalsconceded = matrix(rep(0,2*(n.clubs-1)*n.clubs),ncol=((n.clubs-1)*2))
    teamnames=unique(playing_stat$HomeTeam)
    
    # count goals at Home and Away and create cumulative total per matchweek
    for (t in teamnames){
      HTGC=matrix(rep(0,2*(n.clubs-1)),ncol = 2)
      ATGC=matrix(rep(0,2*(n.clubs-1)),ncol = 2)
      for (i in 1:sum(playing_stat$HomeTeam==t)){
        hgoalconceded=playing_stat[which(playing_stat$HomeTeam==t)[i],c("FTAG")]
        horder=as.numeric(playing_stat[which(playing_stat$HomeTeam==t)[i],c("Date")])
        HTGC[i,]=t(c(hgoalconceded,horder))
      }
      for (i in 1:sum(playing_stat$AwayTeam==t)){
        agoalconceded=playing_stat[which(playing_stat$AwayTeam==t)[i],c("FTHG")]
        aorder=as.numeric(playing_stat[which(playing_stat$AwayTeam==t)[i],c("Date")])
        ATGC[i,]=t(c(agoalconceded,aorder))
      }
  ...
  get_shots=function(playing_stat){
    # Create a dictionary with team names as keys
    shots = matrix(rep(0,2*(n.clubs-1)*n.clubs),ncol=((n.clubs-1)*2))
    teamnames=unique(playing_stat$HomeTeam)
    
    # count goals at Home and Away and create cumulative total per matchweek
    for (t in teamnames){
      HTS=matrix(rep(0,2*(n.clubs-1)),ncol = 2)
      ATS=matrix(rep(0,2*(n.clubs-1)),ncol = 2)
      for (i in 1:sum(playing_stat$HomeTeam==t)){
  ...
  get_shots_on_target=function(playing_stat){
    # Create a dictionary with team names as keys
    shots_on_target = matrix(rep(0,2*(n.clubs-1)*n.clubs),ncol=((n.clubs-1)*2))
    teamnames=unique(playing_stat$HomeTeam)
    
    # count goals at Home and Away and create cumulative total per matchweek
    for (t in teamnames){
      HTST=matrix(rep(0,2*(n.clubs-1)),ncol = 2)
      ATST=matrix(rep(0,2*(n.clubs-1)),ncol = 2)
      for (i in 1:sum(playing_stat$HomeTeam==t)){
  ...
  get_points_gained=function(playing_stat){
    # Create a dictionary with team names as keys
    pointsgained = matrix(rep(0,2*(n.clubs-1)*n.clubs),ncol=((n.clubs-1)*2))
    teamnames=unique(playing_stat$HomeTeam)
    ...
    # count goals at Home and Away and create cumulative total per matchweek
    for (t in teamnames){
      HTP=matrix(rep(0,2*(n.clubs-1)),ncol = 2)
      ATP=matrix(rep(0,2*(n.clubs-1)),ncol = 2)
      for (i in 1:sum(playing_stat$HomeTeam==t)){
  ...
  add_form=function(playing_stat,num){
    ...
    for (i in 1:(num*(n.clubs/2))){
      h[i] = 0  # since form is not available for n MW (n*10)
      a[i] = 0 
    }
    j = num+1
    for (i in (num*(n.clubs/2)+1):nrow(playing_stat)){
      ht = playing_stat$HomeTeam[i]
      at = playing_stat$AwayTeam[i]
  ...
  add_form_df=function(playing_statistics){
    ...
    if(amount.games>=2*(n.clubs/2)){playing_statistics = add_form(playing_statistics,1)}
    if(amount.games>=3*(n.clubs/2)){playing_statistics = add_form(playing_statistics,2)}
    if(amount.games>=4*(n.clubs/2)){playing_statistics = add_form(playing_statistics,3)}
    if(amount.games>=5*(n.clubs/2)){playing_statistics = add_form(playing_statistics,4)}
    if(amount.games>=6*(n.clubs/2)){playing_statistics = add_form(playing_statistics,5)}
  ...
  Standings = read.csv("yearly_updated_data/DE_standings.csv", sep = ",")
  Standings[,1]=str_replace_all(Standings[,1], fixed(" "), "")
  rownames(Standings)=Standings[,1]
  Standings=Standings[,-1]
  Standings[is.na(Standings)]=(n.clubs-2)
  ...
  AvgAge = read.csv("yearly_updated_data/DE_AvgAge.csv", sep = ",")
